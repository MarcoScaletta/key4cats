...
cat_name_dicts = {k: str(v+1) for v, k in enumerate(cat_name_list)}
stats_dict = dict()
...
fig.canvas.manager.set_window_title('Results of 3-internal-behavior/benchmark-removeOne.sh')

with open('result_metrics_3-noquad.csv', 'w') as f:
...
if len(sys.argv) > 1 and sys.argv[1] == "-no-show":
    print("not showing plots, as requested. Check \"result_metrics_3-noquad.png\"")
else:
    plt.show()
