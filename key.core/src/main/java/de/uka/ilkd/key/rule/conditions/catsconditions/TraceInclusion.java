 ...
 */
public class TraceInclusion extends VariableConditionAdapter {
    ...
    public TraceInclusion(SchemaVariable succTr, SchemaVariable anteTr, boolean negated) {
    ...
    public boolean check(SchemaVariable var, SVSubstitute instCandidate, SVInstantiations svInst, Services services) {
        ...
        if(!(succTerm.op() instanceof SchematicTrace))
            return negated;
        if(succTerm.op() == SchematicTrace.SCHEM_TRACE_TRIV)
            return !negated;
